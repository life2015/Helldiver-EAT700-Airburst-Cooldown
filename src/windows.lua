...
return function()
    ...
    function api.pointer_bytes(p)
        assert(p>=0x10000 and p<0x800000000000)
        return ffi.string(ffi.new('uint64_t[1]',p),8)
    end
    function api.writable(p,n)
        local region=ffi.new('EATAirburstMemoryRegion[1]')
    ...
    function api.write(p,expected,data)
        assert(#expected==#data and api.writable(p,#data),'Write target is not private RW data')
    ...
    function api.allocate(data)
        local p=k.VirtualAlloc(nil,#data,0x3000,4)
        assert(p~=nil,'Data allocation failed')
        local n=address(p)
        api.write(n,string.rep('\0',#data),data)
        -- Deliberately process-lifetime memory: native code may retain a table pointer.
        return n
    end
