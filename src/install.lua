return function(create_api,controller,profile,cooldown)
    if rawget(_G,'EATAirburst') then return rawget(_G,'EATAirburst')end
    local state={version=profile.version,status='starting'}
    rawset(_G,'EATAirburst',state)
    ...
    local function report()
        ...
        pcall(function()
            ...
            local f=io.open(dir..'/EATAirburst.log','w');if not f then return end
            f:write('EAT Airburst '..profile.version..'\n')
            for _,key in ipairs({'status','process_id','game_sha256','exe_sha256','tables','base_cooldown','effective_cooldown','error','restored'})do
                f:write(key..'='..tostring(state[key] or '')..'\n')
            end
            f:write('target=EAT-700\nmodes=Flak / Cluster\nnewly_spawned_launcher_required=true\n');f:close()
        end)
    end
    ...
    state.status='waiting_for_tables';report()
    return state
end
