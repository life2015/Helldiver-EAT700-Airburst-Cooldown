...
MODULE='mods/retrox/eat_airburst'
IMPL=MODULE+'_impl'
...
def package_name(channel,version):
    ...
    return f'EAT700-Airburst-{version}-{channel}-{label}.zip'
...
def bridge_source(published,code):
    ...
    return ('local start=(function()\n'+startup+'\nend)()\n'
            ...
            'function() assert(loadstring('+literal(code)+", '@eat_airburst'))() end)\n")
...
def build():
    ...
    text=source();(folder/'eat_airburst.lua').write_text(text,encoding='utf-8')
    lua=Lua(GAME/'bin/lua51.dll')
    try:code=lua.compile(text)
    finally:lua.close()
    (folder/'eat_airburst.luac').write_bytes(code)
    ...
    for channel in ('v14','v15'):
        ...
        description='EAT-700 with RL-77 Flak / Cluster modes and reduced cooldown: base 140s -> 70s (same as EAT-17); 59.85s after 5% and 10% upgrades. EAT-17 remains vanilla. '
        ...
        manifest={'Version':1,'Guid':'9b857302-169c-4d48-b085-2d395ad94a63','Name':f'EAT-700 Airburst {version} {channel}',
                  'Description':description,'Options':[{'Name':'EAT-700 Airburst','Description':description,'Include':[directory]}]}
