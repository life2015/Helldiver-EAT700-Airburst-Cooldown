...
local entry='mods/retrox/eat_airburst'
local impl='mods/retrox/eat_airburst_impl'
...
if MODE=='no_addon' then
    assert(loader.discovery=='0 declared entries' and not calls[entry] and not env.EATAirburst)
else
    assert(loader.discovery=='1 declared entries',tostring(loader.discovery))
    assert(calls[entry]==1)
    if MODE=='missing_impl' then
        assert(loader.modules[entry]:find('EAT Airburst implementation missing',1,true))
        assert(not calls[impl] and not env.EATAirburst)
    else
        assert(loader.modules[entry]=='loaded' and calls[impl]==1)
        assert(env.EATAirburst and env.EATAirburst.status=='stopped')
        -- No game.dll exists in the build process; controlled initialization failure.
        assert(env.EATAirburst.stopped)
...
return 'PASS published v15 discovery + EAT-700 addon: '..MODE
