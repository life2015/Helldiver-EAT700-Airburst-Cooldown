...
for _,installed in ipairs({false,true})do
    ...
    assert(b.EATAirburst and b.EATAirburst.status=='stopped')
    local log=assert(b.logs['mock/EATAirburst-startup.log'])
    ...
    local state=b.EATAirburst;run(BRIDGE,b);assert(state==b.EATAirburst)
...
assert(e.loaded['mods/cowboybingus/armory_preview_cache']==1 and e.EATAirburst)
...
run(BRIDGE,b);assert(b.CowboyBingusModLoader==loader and b.EATAirburst)
...
run(BRIDGE,e);assert(e.EATAirburst and e.EATAirburst.stopped)
